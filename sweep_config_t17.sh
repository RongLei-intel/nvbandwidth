...
export SWEEP_CASE_POLICY_JSON_FILE="$SCRIPT_DIR/sweep_cases_t17.json"
export CUDA_VISIBLE_DEVICES=0
export NUMA_NODE=0
export CPU_BIND=1
export ONLY_TESTCASE=17
...
echo
if [[ $failed -ne 0 ]]; then
	echo "T17 sweep config finished with failures" >&2
else
	echo "T17 sweep config finished successfully"
fi
