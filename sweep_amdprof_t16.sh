...
# Sweep AMD uProf PCM for nvbandwidth testcase 16 over buffer sizes and SM copy widths.
#
# Default command per run:
#   OUTDIR=<sweep_root>/buffer_<N>MiB_smcopy_<B>B BUFFER_SIZE=<N> TESTCASE=16 \
#     EXTRA_NVBW_ARGS="--smCopyBytes <B>" ./collect_amdprof_mem_bw_t33.sh
#
# Useful examples:
#   bash sweep_amdprof_t16.sh
#   BUFFER_SIZES="2 4 8 16" bash sweep_amdprof_t16.sh
#   SM_COPY_BYTES_LIST="4 8 16 32" BUFFER_SIZES="64 128 256" bash sweep_amdprof_t16.sh
#   TARGET_TRANSFER_MIB=131072 bash sweep_amdprof_t16.sh
#   LOOP_COUNT=5000 bash sweep_amdprof_t16.sh
#   PCM_WAIT_FOR_SIGNAL=0 SAMPLE_INTERVAL_MS=5 bash sweep_amdprof_t16.sh
#   USE_SCALED_LOOP_COUNT=1 BASE_LOOP_COUNT=5000 BASE_BUFFER_MIB=2 bash sweep_amdprof_t16.sh
#
...
#   BUFFER_SIZES="1 2 4 8 16 32 64 128 256 512 1024 2048"
...
#   AUTO_LOOP_COUNT=1             choose loopCount from TARGET_TRANSFER_MIB / BUFFER_SIZE
#   TARGET_TRANSFER_MIB=262144    target logical copied MiB per nvbandwidth sample
#   MIN_LOOP_COUNT=16             lower bound for auto/scaled loopCount
#   MAX_LOOP_COUNT=262144         upper bound for auto/scaled loopCount
#   LOOP_COUNT=<N>                explicit fixed loopCount; overrides auto/scaled mode
#   USE_SCALED_LOOP_COUNT=0       legacy mode: scale loopCount inversely with buffer size
...
#   SM_COPY_BYTES=                optional single --smCopyBytes value; supported: 4, 8, 16, 32
#   SM_COPY_BYTES_LIST="4 8 16 32" optional load-size sweep list; overrides SM_COPY_BYTES
...
if [[ -n "$RUN_LABEL" ]]; then
    OUTROOT_DEFAULT="amdprof_nvbw_t16_${RUN_LABEL}_sweep_$(date +%Y%m%d_%H%M%S)"
else
    OUTROOT_DEFAULT="amdprof_nvbw_t16_sweep_$(date +%Y%m%d_%H%M%S)"
fi
...
for sm_copy_bytes in sm_copy_bytes_values:
    for buffer_mib in buffers:
        ...
        row = {
            ...
            'tuning_param_name': info.get('tuning_param_name', 'sm_copy_bytes'),
            'tuning_param_value_bytes': info.get('tuning_param_value_bytes', str(sm_copy_bytes)),
        ...
        row.update({
            ...
            'tuning_param_name': info.get('tuning_param_name', 'sm_copy_bytes'),
            'tuning_param_value_bytes': info.get('tuning_param_value_bytes', str(sm_copy_bytes)),
            'nvbandwidth_SUM_GBps': first_value(summary, ['nvbandwidth_SUM_host_to_device_memcpy_sm_GBps', 'nvbandwidth_SUM_GBps']),
...
md_lines = [
    ...
    '',
    '| SM copy bytes | tuning param | tuning value bytes | Buffer MiB | LoopCount | Status | nvbandwidth SUM | System Mem Rd median | System Mem Rd p95 | System PCIe Rd median | System PCIe Rd p95 | Samples | Result dir |',
    '| ---: | --- | ---: | ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |',
]
for row in rows:
    md_lines.append(
        f"| {cell(row, 'sm_copy_bytes')} | {cell(row, 'tuning_param_name')} | {cell(row, 'tuning_param_value_bytes')} | {cell(row, 'buffer_MiB')} | {cell(row, 'loop_count')} | {cell(row, 'status')} | {cell(row, 'nvbandwidth_SUM_GBps')} | "
