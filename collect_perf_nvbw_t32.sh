...
# Collect AMD pipeline/topdown metrics for nvbandwidth testcase 32.
...
TOPDOWN_METRICS="${TOPDOWN_METRICS:-PipelineL1,PipelineL2,total_dispatch_slots}"
...
perf stat --metric-no-group -p "$pid" \
    -M "$TOPDOWN_METRICS" \
    -e cycles,instructions,task-clock,context-switches,cpu-migrations,page-faults,branches,branch-misses,cache-references,cache-misses \
    -- sleep "$PERF_SECONDS" \
